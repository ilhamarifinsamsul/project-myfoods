<x-layouts.app>
    <div class="grid h-screen place-content-center bg-white font-poppins">
        <img
            src="{{ asset("assets/icons/checkmark-icon.svg") }}"
            alt="Tidak ada data"
            class="w-full overflow-hidden rounded-3xl"
        />
        <div class="mt-4 w-full text-center">
            <p class="text-lg font-semibold text-black-80">
                Pembayaran Berhasil!
            </p>
            <p class="mt-2 text-sm text-black-60 font-medium">
                Terima kasih telah melakukan pembayaran.
            </p>
            <a href="/" wire:navigate class="mt-4 flex w-full items-center justify-center gap-2 rounded-full bg-primary-50 px-6 py-3 font-semibold text-black-10">
                Kembali ke menu
            </a>
        </div>
    </div>
</x-layouts.app>
